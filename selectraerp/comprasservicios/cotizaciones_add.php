...
$valor=$max_progra+1;
*/
$consulta="select * from proveedores where estatus='A' ORDER BY compania";
	...
	<tr>
		...
		<?php
			...
			while($canmateriales=fetch_array($materiales)){
				$iva=$canmateriales['cod_item'];
				$materialesiva = "SELECT iva FROM item WHERE cod_item='".$iva."'";
		...
		?>
		<tr>
			<td width="10" ><input  disabled="true"  type="text"  size="10"  value="<?php echo $canmateriales['cod_item'];?>" />
			<INPUT type="hidden"  name="codigo<?php echo $cantidad?>" id="codigo<?php echo $cantidad?>"  value="<?php echo $canmateriales['cod_item'];?>"/></td></td>
			...
			<td ><input name="precio<?php echo $cantidad?>" id="precio<?php echo $cantidad?>" type="text"  size="10" <?php if(!isset($_POST["guardardatos"])) { echo 'value="0.00"';}?> style="width:100%" onBlur="actualizar_formulario(<?php echo $cantidad ?>,<?php echo $var_por_iva; ?>,<?php echo $totalregistros; ?>);" onKeyPress="javascript:return numeros(event)" /></td>
			<td >
			<input  name="iva_sino_o<?php echo $cantidad?>" id="iva_sino_o<?php echo $cantidad?>" disabled="true" type="text"  size="10" style="width:100%" value="0.00"/>
			<input name="iva<?php echo $cantidad?>" id="iva<?php echo $cantidad?>"  type="hidden" value="0.00" />
