...
$queryReporte = "select s.estado,s.programa,s.descripcion,s.tipo,s.municipio,s.parroquia,
                    ...
                    where year(vp.fecha) = $anio and month(vp.fecha)=$mes and i.cesta_basica = 0 
                    group by vp.siga 
                    order by estado asc ";
...
$objPHPExcel->getActiveSheet()->setTitle('NO BASICO'); 
$objPHPExcel->setActiveSheetIndex(0);
$objPHPExcel->setActiveSheetIndex(0)->getProtection()->setSheet(true);
header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
header('Content-Disposition: attachment;filename="consolidado_no_basico.xlsx"');
