...
class PDF extends FPDF {
    ...
    function Header() {
        ...
        $this->Cell(0, 0, utf8_decode("REPORTE DE PRODUCTOS VENDIDOS CENTRALIZADOS"), 0, 0, 'C');
        ...
        $this->SetWidths(array(15, 15, 15, 70, 15, 15, 15, 15,15,17));
        $this->SetAligns(array("C", "C","C", "C", "C", "C","C", "C", "C","C"));
        $this->Row(array(utf8_decode('SIGA'),utf8_decode('Código de barras'),utf8_decode('Código'), utf8_decode('Descripción'), 'Unid.', 'Total sin IVA','Total con IVA', 'Precio unit sin IVA','Precio unit con IVA','Impuesto'), 1);
    }
    ...
    function ChapterBody() {
        ...
        foreach ($this->array_factura as $reg) {
            ...
            $this->SetLineWidth(0.1);
            $this->SetWidths(array(15, 15, 15, 70, 15, 15, 15, 15,15,17));
            $this->SetAligns(array("C", "C","C", "L", "C", "C","C", "C", "C","C"));
            ...
            $this->Row(array(
                ...
                $reg["unidad"],
                number_format($totsiniva, 2, ',', '.'),
                number_format($totconiva, 2, ',', '.'),
                number_format($prsiniva, 2, ',', '.'),
                number_format($prconiva, 2, ',', '.'),
                $reg["iva"]*100),1);
...
//no se esta viendo el siga en el reporte revisar
 $siga = $_GET["siga"]; 
...
if($siga!="null"){
 ...
 //$array_factura = $comunes->ObtenerFilasBySqlSelect("SELECT * FROM $bdpp.vproducto ");
$pdf = new PDF('P', 'mm', 'A4');
