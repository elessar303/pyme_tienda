...
class PDF extends FPDF {
    ...
    function Header() {
        ...
        $this->Cell(0, 0, utf8_decode("REPORTE DE EXISTENCIA DEL POS"), 0, 0, 'C');
        ...
        $this->SetWidths(array(10,30,90,20,20,20,20,20,20));
        $this->SetAligns(array("C", "C", "C", "C","C", "C", "C", "C","C"));
        $this->Row(array(utf8_decode('N°'), 'Codigo de barra', utf8_decode('Descripción'), 'Exist.', 'Precio S/Iva', 'IVA', 'Precio C/Iva', 'Subtotal', 'Total'), 1);
    }
    ...
    function ChapterBody() {
        ...
        foreach ($this->array_factura as $value) {
            $this->SetFont('Arial', '', 8);
                $coniva=$value["PRICESELL"]+($value["PRICESELL"]*$value["RATE"]);
                $this->SetLineWidth(0.1);
                $this->SetWidths(array(10,30,90,20,20,20,20,20,20));
                ...
                $this->Row(array(
                    $j,
                    $value["CODE"],
                    utf8_decode($value["NAME"].' '.$value["MARCA"].' '.$value["PESO"].$value["MEDIDA"]),
                    number_format($value["UNITS"], 2, ',', '.'),
                    number_format($value["PRICESELL"], 2, ',', '.'),
                    $value["RATE"]*100, 
                    number_format($coniva, 2, ',', '.'),
                    number_format($value["UNITS"]*$value["PRICESELL"], 2, ',', '.'),
                    number_format($value["UNITS"]*$coniva, 2, ',', '.')
...
$array_factura = $comunes->ObtenerFilasBySqlSelect("SELECT UNITS,REFERENCE,
    i.descripcion1 AS NAME,m.marca AS MARCA,i.pesoxunidad AS PESO,um.nombre_unidad AS MEDIDA,
    CODE, PRICESELL, RATE
FROM $pos.stockcurrent a
LEFT JOIN $pos.products AS b ON b.id = a.product
LEFT JOIN $pyme.item i ON i.codigo_barras = b.CODE
LEFT JOIN $pyme.marca m ON m.id = i.id_marca
LEFT JOIN $pyme.unidad_medida um ON um.id = i.unidadxpeso
LEFT JOIN $pos.taxes as tax ON b.TAXCAT=tax.CATEGORY
WHERE a.units <>0
ORDER BY NAME");
$pdf = new PDF('L', 'mm', 'A4');
