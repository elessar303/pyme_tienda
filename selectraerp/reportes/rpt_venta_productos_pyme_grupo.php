...
class PDF extends FPDF {
    ...
    function Header() {
        ...
        $this->Cell(0, 0, utf8_decode("REPORTE DE PRODUCTOS VENDIDOS POR CATEGORIA"), 0, 0, 'C');
        ...
        $this->Row(array( 'Cod. Grupo', 'Descripcion','IVA', 'Monto Vendido.', 'Unid. Vendidas'), 1);
    ...
    function ChapterBody() {
        ...
        foreach ($this->array_factura as $id => $reg) {
            ...
            $this->Row(array($reg["CODE"],$reg["NAME"],number_format($reg["_item_piva"], 0, ',', '.')."%",number_format($total, 2, ',', '.'),number_format($reg["UNITS"], 2, ',', '.')),1);
        ...
        $this->Row(array($i, "T O T A L E S", $total_venta, $unids), 1);
...
$sql="SELECT grupo.descripcion as NAME,
  grupo.cod_grupo as CODE,
  factura_detalle._item_preciosiniva AS PRICESELL,
  sum(factura_detalle._item_cantidad) as UNITS,
  factura.fechaFactura as DATENEW,
...
INNER JOIN grupo on item.cod_grupo=grupo.cod_grupo     
WHERE factura.fechaFactura BETWEEN '{$inicio} 00:00:00' AND '{$final} 23:59:59'
