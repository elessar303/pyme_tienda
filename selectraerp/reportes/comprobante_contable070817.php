...
$datoscamposingresos=$comunes->ObtenerFilasBySqlSelect("select sum(b.monto) as monto, c.descripcion as tipo_venta from comprobante as a inner join ingresos_detalles as b on a.id=b.id_comprobante inner join departamentos as c on b.tipo_ingreso=cod_departamento where date(a.fecha)='".$fecha[0]['fecha']."' and (a.banco is null || a.banco='0.00') group by cod_departamento");
