<?php
/*
 * Actualizaciones y mejoras:  jm
 * e-mail: jmpulgar@asys.com.ve
 */
...
class PDF extends FPDF {
	...
	function Header() {
		...
		$this->SetWidths(array(50,20,40,20,40,20,25));
		...
		$this->Row(array('Compania','R.I.F.','Cant. Fact. Pendientes por Pagar','Monto','Cant. Fact. Pagadas','Monto', 'TOTAL'),1);
		$this->SetX(8);
		...
		*/
		...
		while($this->array_factura[$i])
		{
			...
			*/
			...
			//$sql = $comunes2->ObtenerFilasBySqlSelect("select prov.*, factura.*,count(factura.id_factura) as facturas, sum(factura.total_a_pagar) as montooo FROM proveedores prov INNER JOIN cxp_factura  factura ON prov.id_proveedor = factura.id_proveedor where factura.cod_estatus='1' and factura.fecha_factura BETWEEN '".$_GET["fecha"]."' and '".$_GET["fecha_hasta"]."'  and  factura.id_proveedor='".$this->array_factura[$i]["id_proveedor"]."' GROUP BY prov.descripcion asc");


			// CONSULTA LAS CUENTAS PAGADAS
			$comunes3 = new ConexionComun();
			$sql2 = $comunes3->ObtenerFilasBySqlSelect("SELECT prov.*, factura.*, COUNT(factura.cod_factura) AS facturaspagadas, SUM(factura.monto_total_con_iva) AS montopagado FROM proveedores prov INNER JOIN cxp_factura factura ON prov.id_proveedor = factura.id_proveedor  where factura.fecha_factura BETWEEN '".$_GET["fecha"]."' and '".$_GET["fecha_hasta"]."'  and factura.cod_estatus='2' and factura.id_proveedor='".$this->array_factura[$i]["id_proveedor"]."' GROUP BY prov.descripcion asc");


			// CONSULTA las cuentas pendiente por pagar
			$comunes4 = new ConexionComun();
			$sql3 = $comunes4->ObtenerFilasBySqlSelect("SELECT prov.*, factura.*, COUNT(factura.cod_factura) AS facturas, SUM(factura.monto_total_con_iva) AS montoo FROM proveedores prov INNER JOIN cxp_factura factura ON prov.id_proveedor = factura.id_proveedor WHERE  factura.fecha_factura BETWEEN '".$_GET["fecha"]."' and '".$_GET["fecha_hasta"]."' and factura.cod_estatus='1' and factura.id_proveedor='".$this->array_factura[$i]["id_proveedor"]."' GROUP BY prov.descripcion asc");
...
//echo "SELECT prov.*, factura.*, COUNT(factura.id_factura) AS facturas, SUM(factura.total_a_pagar) AS montoo FROM proveedores prov INNER JOIN cxp_factura factura ON prov.id_proveedor = factura.id_proveedor  WHERE  fecha_factura BETWEEN '".$_GET["fecha"]."' and '".$_GET["fecha_hasta"]."' GROUP BY prov.descripcion asc";
$array_factura = $comunes->ObtenerFilasBySqlSelect("SELECT prov.*, factura.*, COUNT(factura.id_factura) AS facturas, SUM(factura.total_a_pagar) AS montoo FROM proveedores prov INNER JOIN cxp_factura factura ON prov.id_proveedor = factura.id_proveedor  WHERE  fecha_factura BETWEEN '".$_GET["fecha"]."' and '".$_GET["fecha_hasta"]."' GROUP BY prov.descripcion asc");
