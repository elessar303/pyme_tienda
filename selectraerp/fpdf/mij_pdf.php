...
function imprimir_tabla ($pdf,$nivel)
{
	...
	$consulta = "SELECT Sec, Denominacion FROM cwsector where Recno <>99 ";
	...
	$this->Row(array('Sector',utf8_decode('Denominación'),'Asignacion Original','Modificacion','Asinacion Ajustada','Compromisos','Saldo para Comprometer','Gastos Causados','Gastos Pagados'));
	...
	while ($total>=$contar)
	{
		$fila = fetch_array($rs);
		$conexion=conexion();
		$consulta2 = "SELECT SUM(Inicial) as Inicial,SUM(Monto) as Monto,SUM(aumento ) as aumento,SUM(disminucion) as disminucion,SUM(Acucom) as comp, SUM(Acucau) as causado, SUM(Acupag) as pagado, SUM(Dispo) as dispo FROM cwprepar WHERE Sector = '".$fila['Sec']."' ";
		$resultado2 = query($consulta2,$conexion);
		$fetch = fetch_array($resultado2);
		
		$consulta3 = "SELECT SUM(Monto) AS Monto FROM cwpreeje WHERE Sector = '".$fila['Sec']."' AND  Fecha BETWEEN '".$this->desde."' AND '".$this->hasta."' ";
		$resultado3 = query($consulta3,$conexion);
		$fetch3 = fetch_array($resultado3);
		
		$consulta4 = "SELECT SUM(Monto) AS Monto FROM cwpreejc WHERE Sector = '".$fila['Sec']."' AND  Fecha BETWEEN '".$this->desde."' AND '".$this->hasta."' ";
		$resultado4 = query($consulta4,$conexion);
		$fetch4 = fetch_array($resultado4);
		
		$consulta7 = "SELECT SUM(Monto) AS Monto FROM cwpreeje WHERE Sector = '".$fila['Sec']."' AND  Fecha BETWEEN '".$this->desde."' AND '".$this->hasta."' AND Cheque <> 0 ";
		$resultado7 = query($consulta7,$conexion);
		$fetch7 = fetch_array($resultado7);
		
		$consulta5 = "SELECT Monto, RecNoOrders  FROM cwpreeje WHERE Sector = '".$fila['Sec']."' AND  Fecha BETWEEN '".$this->desde."' AND '".$this->hasta."' AND Cheque = 0 ";
		...
		$mod = $aumento-$disminucion;
		$mods = number_format($mod,2,',','.');
		if($mod<0)
			$mods = number_format($mod*-1,2,',','.');
		$descripcion = $fila['Denominacion'];
		$montoAct = $montoIni + $mod;
		...
		$this->Setancho(array(5,5,5,5,5,5,5,5,5));
		$this->SetAligns(array('L','L','R','R','R','R','R','R','R'));
		$this->Row(array($codigo,utf8_decode($descripcion),$montoInicial,$valor,$montoActualizado,$montoComprometido,$montoDisponible,$montoCausado,$montoPagado));
		$cont++;
	...
	$this->Row(array('TOTAL ',$totMontoInicial,number_format($totAumen - $totDismi,2,',','.'),$totMontoActualizado,$totMontoComprometido,$totMontoDisponible,$totMontoCausado,$totMontoPagado));
	$this->Ln();
}
function Footer()
{
