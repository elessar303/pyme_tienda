...
function imprimir_datos($pagina,$num_paginas,$fechaDesde,$fechaHasta)
{
	...
	$this->SetFont('Arial','B',12);
     	$this->Cell(35);
        $string=utf8_decode('COMPROBANTE DE RETENCIÓN DE RESPONSABILIDAD SOCIAL ');
...
function imprimir_tabla($pdf,$pagina,$num_paginas,$fechaDesde,$fechaHasta){
	...
	$conexion=conexion();
	$consulta = "SELECT * FROM ordenes_pago WHERE estado <> 'Anulada' AND monto_aporte<>'0'  AND cheque<>'0' AND fecche BETWEEN '".$fechaDesde."' AND '".$fechaHasta."' ORDER BY cuenta, fecche";
	...
	$consulta7 = "SELECT DISTINCT(cuenta) FROM ordenes_pago WHERE monto_aporte<>'0'  AND cheque<>'0' AND fecche BETWEEN '".$fechaDesde."' AND '".$fechaHasta."'";
	$rs7 = query($consulta7,$conexion);
	$fetch7 = fetch_array($rs7);

	//Cabecera
	//Cabecera
			$this->Cell(95,7,'Beneficiario','LTB',0,'L');
			$this->Cell(30,7,'RIF','TB',0,'C');
			$this->Cell(25,7,'Fecha ch.','TB',0,'C');
			$this->Cell(35,7,'Cheque','TB',0,'C');
			$this->Cell(25,7,'Base','TB',0,'C');
			$this->Cell(25,7,'Monto Pagado','TB',0,'C');
			$this->Cell(25,7,'Monto RRS','TBR',1,'C');
			//$this->Cell(25,7,'% Ret.','TRB',1,'C');
	//Datos
	$cont=1;
	$contar=1;
	$pagina=1;
	...
	$row_rs = fetch_array($rs);
	$reg++;
		if($row_rs['cuenta'] != $cuenta7)
			{
				if(($subislr != 0))	
				{
					$this->Cell(250,5,'Subtotal RRS retenido por cuenta:'.number_format($subislr, 2, ',', '.'),0,1,'R');
					$subislr = 0;
				}
			
				$cuenta7 = $row_rs['cuenta'];
				$consulta8 = "SELECT descripcion FROM bancos WHERE cuenta = '".$cuenta7."'";
				$rs8 = query($consulta8,$conexion);
				$fetch8 = fetch_array($rs8);
				$this->SetFont('Times','B',12);
				$this->Cell(120,5,$fetch8['descripcion'],0,0,'L');
				$this->Cell(120,5,'Cuenta:'.$row_rs['cuenta'],0,1,'L');
				$this->SetFont('Times','',12);
				$cont = $cont + 2;
			}
	 // llamado para hacer multilinea sin que haga salto de linea
        $this->SetWidths(array(95,30,25,35,25,25,25,25));
	$this->SetAligns(array('L','C','C','C','R','R','R','C'));
        $this->Row(array($row_rs['bene'],$row_rs['rif'],fecha($row_rs['fecha']),$row_rs['cheque'],number_format($row_rs['monto_base'], 2, ',', '.'),number_format($row_rs['montopago'], 2, ',', '.'),number_format($row_rs['monto_aporte'], 2, ',', '.')));
	$totalMonto += $row_rs['monto_base'];
	$totalMontoPago += $row_rs['montopago'];
	$totalIVA += $row_rs['monto_aporte'];
	$totalRet += $row_rs['monto_aporte'];
	$subislr +=$row_rs['monto_aporte'];
	...
	{	
		if ($pagina!=$num_paginas){
			...
			$this->Cell(25,7,'Base','TB',0,'C');
			$this->Cell(25,7,'Monto Pagado','TB',0,'C');
			$this->Cell(25,7,'Monto ISLR','TBR',1,'C');
			//$this->Cell(25,7,'% Ret.','TRB',1,'C');
			$cont=1;
		}
	}else{$cont++;}
	...
	if($cont<=$cantidad_registros)
	{
		$this->Cell(185,7,'TOTALES GENERALES Bs. ------------------------------>',0,0,'L');
		$this->Cell(25,7,number_format($totalMonto, 2, ',', '.'),0,0,'R');
		$this->Cell(25,7,number_format($totalMontoPago, 2, ',', '.'),0,0,'R');
		$this->Cell(25,7,number_format($totalIVA, 2, ',', '.'),0,0,'R');
		$this->Cell(25,7,'',0,1,'C');
