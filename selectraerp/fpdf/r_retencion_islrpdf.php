...
function imprimir_datos($pagina,$num_paginas,$fechaDesde,$fechaHasta)
{
	...
	$this->SetFont('Arial','B',12);
     	$this->Cell(35);
        $string=utf8_decode('COMPROBANTE DE RETENCION DE I.S.L.R. ');
...
function imprimir_tabla($pdf,$pagina,$num_paginas,$fechaDesde,$fechaHasta){
	...
	$conexion=conexion();
	$consulta = "SELECT * FROM ordenes_pago WHERE estado <> 'Anulada' AND monto_islr<>'0'  AND cheque<>'0' AND fecche BETWEEN '".$fechaDesde."' AND '".$fechaHasta."' ORDER BY cuenta, fecche";
	...
	$consulta7 = "SELECT DISTINCT(cuenta) FROM ordenes_pago WHERE monto_islr<>'0' AND cheque<>'0' AND fecche BETWEEN '".$fechaDesde."' AND '".$fechaHasta."'";
	$rs7 = query($consulta7,$conexion);
	$fetch7 = fetch_array($rs7);

	//Cabecera
	//Cabecera
			$this->Cell(80,7,'Beneficiario','LTB',0,'L');
			...
			$this->Cell(25,7,'Monto ISLR','TB',0,'C');
	...
	$reg++;
		if($row_rs['cuenta'] != $cuenta7)
			{
				if(($subislr != 0))	
				{
					$this->Cell(250,5,'Subtotal islr retenido por cuenta:'.number_format($subislr, 2, ',', '.'),0,1,'R');
					$subislr = 0;
				}
	 ...
	 // llamado para hacer multilinea sin que haga salto de linea
        $this->SetWidths(array(80,30,25,25,25,25,25,25));
	$this->SetAligns(array('L','C','C','C','R','R','R','C'));
        $this->Row(array($row_rs['bene'],$row_rs['rif'],fecha($row_rs['fecha']),$row_rs['cheque'],number_format($row_rs['monto_base'], 2, ',', '.'),number_format($row_rs['montopago'], 2, ',', '.'),number_format($row_rs['monto_islr'], 2, ',', '.'),number_format($row_rs['porce_reten'], 2, ',', '.')));
	$totalMonto += $row_rs['monto_base'];
	$totalMontoPago += $row_rs['montopago'];
	$totalIVA += $row_rs['monto_islr'];
	$totalRet += $row_rs['monto_islr'];
	$subislr +=$row_rs['monto_islr'];
	...
	if($cont<=$cantidad_registros)
	{
		$this->Cell(160,7,'TOTALES GENERALES Bs. ------------------------------>',0,0,'L');
