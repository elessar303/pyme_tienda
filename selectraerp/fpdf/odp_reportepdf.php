...
function totales($fila_odp,$moneda){
	...
	$this->SetFont('Arial','I',8);
	$this->Ln();
	$this->Cell(10,5,number_format(($valuacion+$iva),2,",",".").' -  Sub-Total '.$moneda,0,0,'L');
	$this->Ln();
	...
	$this->SetFont('Arial','I',8);
	$this->Ln();
	$this->Cell(10,5,number_format($fondoAS,2,",",".").'-  Retencion por Aporte Social',0,0,'L');
	$this->Ln();
...
function datos_partidas($nro_odp,$periodo,$nro_odp,$fila_odp, $moneda,$pdf)
{
	...
	if ($cont+5>$cantidad_registros){
		...
		$this->SetFont("Arial","B",8);
		$this->Ln(2);
		$this->Cell(188,5,utf8_decode('PARTIDAS PRESUPUESTARIAS EJERCICIO FISCAL '.date("Y")),1,0,'C');
		$this->Ln();
