...
class PDF extends FPDF {
	...
	function Header() {
		...
		$this->Ln(10);
		$this->Cell(10);
		$this->Cell(160, 8, 'LISTADO DE MATERIALES', 0, 0, "C");
	...
	function imprimir_datos($nro_odp, $fila_odp, $moneda, $pdf) {
		...
		$this->Cell(18, 7, 'Monto Min.', 'LTB', 0, 'C');
		$this->Cell(18, 7, utf8_decode('Monto Máx.'), 'LTBR', 0, 'C');
		...
		while ($totalwhile >= $contar) {
			...
			$busq = query("SELECT a.cantidad FROM item i,item_existencia_almacen a where i.id_item=a.id_item and i.id_item=" . $var_codigo, $conexion);
			if (num_rows($busq) != 0) {
