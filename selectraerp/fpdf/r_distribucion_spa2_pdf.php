...
function Header()
{
	...
	$this->Cell(260,5,utf8_decode('DISTRIBUCIÓN POR SECTOR, PROGRAMA Y ACTIVIDAD (SUB-ESPECÍFICAS)'),0,1,'C');
	...
	$this->MultiCell(250,5,utf8_decode('Proyecto o Acción Especifica: '.$codPrograma.'-'.$desPrograma),0,'L');
	$this->MultiCell(250,5,utf8_decode('Acción Centralizada: '.$codActividad.'-'.$desActividad),0,'L');
...
function imprimir_tabla ($pdf,$nivel)
{
	...
	$conexion=conexion();
	$consulta = "SELECT * FROM cwprepar LEFT JOIN cwprecue ON cwprepar.Codigo=cwprecue.CodCue WHERE cwprecue.Tipocta='".$nivel."' AND cwprepar.Sector='99' AND cwprepar.Programa='".$pdf->programa."' AND cwprepar.Actividad='".$pdf->actividad."' ORDER BY cwprepar.Codigo";
	...
	while ($total>=$contar)
	{
		...
		$this->SetAligns(array('C','C','L','R','R','R','R','R'));
		$this->Row(array($pdf->programa.$pdf->actividad,$codigo,utf8_decode($descripcion),$montoInicial,$modificaciones,$montoActualizado,$montoComprometido,$montoDisponible));
