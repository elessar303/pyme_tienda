...
//  	$this->Cell(188,5,'',0,0);
	...
	$this->Ln(17);
	$this->Cell(50,7,'',0,0);
	$this->Cell(138,7,utf8_decode($nombreContribuyente),0,1,'L');
 	$this->Ln(1);
	$this->Cell(35,8,'',0);
	$calculo=strlen($montoLetras)/60;
	if($calculo>=0 && $calculo<=1){
		$this->MultiCell(145,6,$montoLetras,0,'L');
		$this->Cell(120,6,'',0,1);
	}else{
		$this->MultiCell(145,6,$montoLetras,0,'L');
	}
	...
	$this->Ln(2);
	$this->Cell(80,5,"        $ciudad, ".$dia." de ".mesaletras($mes),0,0);
	$this->Cell(108,5,'  '.$ano,0,0,'L');
	$this->Ln(10);
...
function datos_cheque($fila3,$oc,$odp,$fecha,$fA,$odpfecha){
	$conexion = conexion();
	//Banco
	$idbanco=$fila3['banco'];
	...
	$this->Ln(17);
	$this->Cell(188,20,'',0,1);
	$this->Cell(188,7,'',0,1);
	$this->Cell(34,5,'',0,0);
	$this->Cell(154,5,utf8_decode($fila3['beneficiario']),0,1,'C');
	$this->Cell(34,5,'',0,0);
	$this->Cell(154,5,$fila3['cedula'],0,1,'C');
	$this->Cell(34,5,'',0,0);
	$this->MultiCell(154,5,utf8_decode($fila3['concepto']),0,'J');

	$lineas=strlen($fila3['concepto'])/70;
//  	$this->Cell(34,5,$lineas,0,1);
...
$this->Ln(3);
	$this->Cell(154,5,'',0,0);
	$this->SetFont('Sanserif','',10);
	$this->Cell(34,5,'**'.number_format($fila3['monto'],2,',','.').'**',0,1,'C');
$this->Ln(3);	
	$this->Cell(154,10,'',0,1);
	$this->Cell(154,8,'',0,1);
	...
	list($anio,$mes,$dia)=explode("-",$fechaemi);
   	$fecha2= $dia."/".$mes."/".$anio; 
// 	$this->Cell(2,5,'',0,0,'C');
	$this->Cell(21,5,$fecha,0,0,'C');
	$this->Cell(21,5,$odp,0,0,'C');
	$this->Cell(25,5,$fecha2,0,0,'C');
	$this->Cell(24,5,$numero,0,0,'C');
	$this->Cell(51,5,$nombreBanco,0,0,'C');
	$this->Cell(50,5,$cuenta,0,1,'C');
	/*
	$this->Ln();
...
$banco=@$_GET["banco"];
	...
	$pdf->datos_cheque($fila3,$oc,$odp,$fecha,$fA,$odpfecha);
