...
if($num[0]['contar']==0)
{
    ...
    $insertar2=$conn->ExecuteTrans(
        ...
        VALUES 
        ('".$nro_cataporte."', '3', '".$fecha."',".$_SESSION['cod_usuario'].", ".$monto_usuario.", ".$observacion.", '".$_POST['banco']."')
        ");
	foreach ($_POST['nro_depositos'] as $key) 
    {
        $num1=$conn->ObtenerFilasBySqlSelect("Select monto_acumulado_deposito from caja_principal where nro_deposito='".$key."'");
        $acumuladototal=$num1[0]['monto_acumulado_deposito']-$monto_usuario;
        if($monto_usuario==0)
        {
        ...
        $insertar3=$conn->ExecuteTrans(
            ...
            (select (id_deposito+1), '".$correlativo."', 0, monto_acumulado, '".$nro_cataporte."', now(), cod_banco, '".$login->getNombreApellidoUSuario()."', 0, monto_acumulado_tarjeta,  ".$monto_usuario.", ".$acumuladototal.", 0, monto_acumulado_tickets,  0, monto_acumulado_cheque, 0, 0  from caja_principal where nro_deposito='".$key."')
            ");

        //se modifica depositos de esas cajas principales
        $select_depositos=$conn->ObtenerFilasBySqlSelect("select * from caja_principal_depositos where id_caja_principal='".$_POST['nro_depositos'][0]."'");
        if(isset($select_depositos) && $select_depositos!='')
        {
            ...
            //modificando el estatus
            $conn->ExecuteTrans("update caja_principal_depositos set estatus=1 where id='".$_POST['id_detalle_deposito']."'");
        }
