...
if (isset($_POST["transaccion"])) {
    ...
    $id_ubicacion=$clientes->ObtenerFilasBySqlSelect("select id_ubicacion from parametros_generales");
    $factura->BeginTrans();
    ...
    $sql = "INSERT INTO `despacho_new` (
        ...
        `usuario_creacion`,`cod_estatus`,`formapago`, `impresora_serial`, `money`, `facturacion`, `electronico`
        )
    VALUES(
        ...
        '{$usuario}', '{$cod_estatus}', '{$_POST["forma_pago"]}', '".impresora_serial."' , '".$money[0]['money']."','{$_POST["facturacion"]}', '{$_POST["electronico"]}'
        );";
  ...
  /*  $insert_modulo_caja = "INSERT INTO 
    ...
    */
    ...
    for ($i = 0; $i < (int) $_POST["input_cantidad_items"]; $i++) {
        /*$detalle_item_instruccion = "
            ...
            VALUES (
                ...
                if ($_POST["electronico"]==1 && $_POST["_item_piva"][$i]==12 && $_POST["input_totalizar_sub_total"]<=2000000) {
                    $_POST["_item_piva"][$i]=9;
                }
                if ($_POST["electronico"]==1 && $_POST["_item_piva"][$i]==12 && $_POST["input_totalizar_sub_total"]>2000000) {
                    $_POST["_item_piva"][$i]=7;
                }
    ...
    $parametros_impresora_fiscal[0]['tipo_facturacion'] = 0;
    ...
    if($erroImpresora==1){
        ...
        $valorImpresora= 1;    
         ...
         $sql="SELECT sum(`_item_cantidad`*`_item_preciosiniva`*`_item_piva`)/100 + sum(`_item_cantidad`*`_item_preciosiniva`) as total_nuevo FROM `despacho_new_detalle` WHERE `id_factura`=".$id_facturaTrans."";
        $total_nuevo=$clientes->ObtenerFilasBySqlSelect($sql);
        $factura->Execute2("UPDATE despacho_new SET totalizar_total_general=".$total_nuevo[0]['total_nuevo']."  WHERE id_factura=".$id_facturaTrans."");
    }
   ...
   if ($id_cliente==0) {
$ubica=$clientes->ObtenerFilasBySqlSelect("select * from estados_puntos");
...
$punto = $clientes->ObtenerFilasBySqlSelect("SELECT `nombre_punto`,codigo_siga_punto as siga  from puntos_venta where estatus='A'");
...
//comienza el proceso de las cestas
$ubicaciones  = $clientes->ObtenerFilasBySqlSelect("select cod_almacen, id_ubicacion from parametros_generales");
//buscar las cestas
$cestas  = $clientes->ObtenerFilasBySqlSelect("select * from cesta_clap");
$valueSELECT = "";
$outputSELECT = "";
$bandera=1;
foreach($cestas as $key => $item)
{
    //buscar el detalle de la cesta
    $verificar= $clientes->ObtenerFilasBySqlSelect("select * from cesta_clap_detalle where id_cesta=".$item['id']);
    foreach ($verificar as $key => $value) 
    {
        //verificar si el producto esta en existencia de no ser asi no se agrega al combo
        $comparar=$clientes->ObtenerFilasBySqlSelect(
            "select * from item_existencia_almacen where cod_almacen=".$ubicaciones[0]['cod_almacen']."
            and id_ubicacion=".$ubicaciones[0]['id_ubicacion']." and id_item=".$value["id_item"]. " and
            cantidad>=".$value['cantidad']*$item['cantidad']);

         
      
      
        if($comparar[0]['id_item']==null)
        {
            $bandera=0;
        }
    }//fin del foreach
...
$smarty->assign("option_values_cesta",$valueSELECT);
$smarty->assign("option_output_cesta",$outputSELECT);
