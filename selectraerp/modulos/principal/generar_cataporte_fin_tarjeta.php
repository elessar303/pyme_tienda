...
if($num[0]['contar']==0)
{
    ...
    $insertar2=$conn->ExecuteTrans(
        ...
        VALUES 
        ('".$nro_cataporte."',".$count.", '2', '".$fecha."',".$_SESSION['cod_usuario'].", ".$monto_usuario.", ".$observacion.")
        ");
	foreach ($_POST['nro_depositos'] as $key) 
    {
        $num1=$conn->ObtenerFilasBySqlSelect("Select monto_acumulado_tarjeta from caja_principal where nro_deposito='".$key."'");
        ...
        $insertar3=$conn->ExecuteTrans(
            ...
            (select (id_deposito+1), '".$correlativo."', monto, monto_acumulado, '".$nro_cataporte."', now(), cod_banco, '".$login->getNombreApellidoUSuario()."', ".$monto_usuario.", ".$acumuladototal.", monto_deposito, monto_acumulado_deposito, monto_tickets, monto_acumulado_tickets,  monto_cheque, monto_acumulado_cheque, monto_credito, monto_acumulado_credito  from caja_principal where nro_deposito='".$key."')
            ");
