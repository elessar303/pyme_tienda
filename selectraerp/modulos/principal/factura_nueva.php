...
if (isset($_POST["PFactura2"])) {
    ...
    for ($i = 0; $i < (int) $_POST["input_cantidad_items"]; $i++) {
    	...
    	if($seriales==1)
    	{
    		...
    		if($kk==0)
    		{
				 $factura->ExecuteTrans("insert into despacho (cod_despacho, id_factura, fecha_creacion, usuario, cantidad_item, estatus) values ('0000', '$id_facturaTrans', CURRENT_TIMESTAMP, '{$usuario}','$cantt',0)");
    ...
    $parametros_impresora_fiscal = $parametros->ObtenerFilasBySqlSelect("SELECT tipo_facturacion, swterceroimp, impresora_marca, impresora_serial, porcentaje_impuesto_principal, iva_a, iva_b, iva_c FROM parametros_generales;");
        ...
        if ($parametros_impresora_fiscal[0]['swterceroimp'] == 1) {
            ...
            switch ($parametros_impresora_fiscal[0]['impresora_marca']) {
                ...
                case "vmax":                  
                    ...
                    $cabecera = "<ABRIR_CF," .$_POST['nombre']. "," .$_POST['id_fiscal']. ">\n";
                  
                    fwrite($archivo, $cabecera);
                    $detalles = "";
                    foreach ($lineas as &$linea) {
                        $detalles = "<ITEM_CF," .$linea["descripcion"]. "," .$linea["cantidad"]. "," .$linea["precio"]. "," .$linea["iva"] . "," . "0" . "," . "0" . ">" . "\n";
                        fwrite($archivo, $detalles);
                    }
                    $resumen.= "<SUBTOTAL_CF>\n";
                    $resumen.="<PAGO_CF," .number_format($_POST["totalizar_monto_cancelar"], 2, ",", "") . ">\n";
                    $resumen.= "<CERRAR_CF>\n";
