...
$consulta = "
SELECT *,time(f.fecha_creacion) as hora FROM factura f
...
$consulta = "SELECT * FROM factura_detalle fdet
  ...
  LEFT JOIN item i ON i.id_item = fdet.id_item
  WHERE fdet.id_factura = '{$dataDevolucion[0]["id_factura"]}'";
...
$parametros_generales = $parametros->ObtenerFilasBySqlSelect("SELECT tipo_facturacion, swterceroimp, impresora_serial, impresora_marca, porcentaje_impuesto_principal, iva_a, iva_b, iva_c, nombre_impuesto_principal FROM parametros_generales;");
...
if (isset($_POST["anularFactura"])) {
    ...
    $factura->BeginTrans();
    $cod_estatus = "3";
//devolucion de serial daniel 6/4/15
    $id_fact=$dataDevolucion[0]["id_factura"];
   $consulta1 = "
   SELECT dd. *,d.* FROM despacho d
   INNER JOIN despacho_detalle dd ON dd.id_despacho=d.id
    WHERE id_factura = '{$id_fact}'";

    $dataDespacho = $factura_devolucion->ObtenerFilasBySqlSelect($consulta1);
    ($dataDespacho);
    foreach ($dataDespacho as $key => $value) {
        $factura_devolucion->ExecuteTrans("UPDATE item_serial  SET estado = '1' WHERE id_producto = '".$value["id_item"]."' and serial = '".$value["serial"]."'");
    }
...
//++++++++++++++++++++++++++++++++++++++++
    ...
    $credito2= $dataDevolucion[0]["totalizar_monto_credito2"];
    $deposito = $dataDevolucion[0]["totalizar_monto_deposito"];
    ...
    for ($i = 0; $i < $cantItems; $i++) {
        ...
        $item = mysql_fetch_array(mysql_query("SELECT cod_item, descripcion2 FROM item WHERE id_item = {$dataDetalleFactura[$i]["id_item"]}"));
        $codigo_item = $item['cod_item'];
        //echo $codigo_item;exit();
        ...
        if ($item['descripcion2'] != "") {# antes $descrip_producto[0]['descripcion2']
        ...
        // $campos = $almacen->ObtenerFilasBySqlSelect("SELECT * FROM ubicacion where devolucion='1'" );   
        $campos = $almacen->ObtenerFilasBySqlSelect("SELECT * FROM ubicacion where puede_vender='1'" );   
        $id_ubicacion= $campos[0]["id"];
        $id_almacen1= $campos[0]["id_almacen"];

         $existenciaUbic = $almacen->getFilas();

        $kardex_almacen_detalle_instruccion = "
            INSERT INTO kardex_almacen_detalle (
                `id_transaccion`, `id_almacen_entrada`,
                `id_almacen_salida`, `id_item`, `cantidad`, `id_ubi_entrada`)
            VALUES (
                '{$id_transaccion}', '{$dataDetalleFactura[$i]["_item_almacen"]}', '{$dataDetalleFactura[$i]["_item_almacen"]}',
                '{$dataDetalleFactura[$i]["id_item"]}', '{$dataDetalleFactura[$i]["_item_cantidad"]}','".$id_ubicacion."');";
        ...
        $campos = $factura_devolucion->ObtenerFilasBySqlSelect("
                        ...
                        WHERE id_item  = '{$dataDetalleFactura[$i]["id_item"]}' AND cod_almacen = '".$id_almacen1."' AND id_ubicacion='". $id_ubicacion."'");
        $cantidadExistente = $campos[0]["cantidad"];
        $cantidadFilas = $factura_devolucion->getFilas();
        ...
        $insertarExistencia="INSERT INTO item_existencia_almacen (
                `cod_almacen`, `id_item`,
                `cantidad`, `id_ubicacion`)
            VALUES (
                 '".$id_almacen1."','{$dataDetalleFactura[$i]['id_item']}', '{$dataDetalleFactura[$i]['_item_cantidad']}','".$id_ubicacion."');";
              $factura_devolucion->ExecuteTrans($insertarExistencia);
    ...
    if ($parametros_generales[0]['tipo_facturacion'] == 1) {
        #Si usa Spooler Fiscal
        if ($parametros_generales[0]['swterceroimp'] == 1) {
            /*
            ...
            switch ($parametros_generales[0]['impresora_marca']) {
                case "hasar":
                    include ("../../libs/php/clases/hasar/HasarPHP.php");
                    $objHasar = new HasarPHP();

                    $NR = $parametros_generales[0]['impresora_serial'];
                    ...
                    $TD = "D";
                    $PI = $objHasar->PI;
                    $FD = $NR . "-" . $dataDevolucion[0]["cod_factura_fiscal"];

                    $fp = fopen($objHasar->file, "w");

                    $write = fputs($fp, $BS . $FS . "1" . $LF);
                    $write = fputs($fp, $TH . $FS . "1" . $FS . "Ref. Interna: " . $nro_devolucion . $LF);
                    $write = fputs($fp, $DF . $FS . $datos_cliente[0]['nombre'] . $FS . $datos_cliente[0]['rif'] . $FS . $FD . $FS . $NR . $FS . $FS . $FS . $TD . $LF);

                    foreach ($lineas as $linea) {

                        $write = fputs($fp, $PI . $FS .
                                trim($linea["descripcion"]) . $FS .
                                number_format($linea["cantidad"], 2, ".", "") . $FS .
                                /* number_format($linea["precio"], 2, ".", "") */str_replace(",", ".", $linea["precio"]) . $FS .
                                /* number_format($linea["iva"], 2, ".", "") */str_replace(",", ".", $linea["iva"]) . $FS .
                                "M" . $FS .
                                $linea["codigo_item"] . $LF);
                    ...
                    if (!$pago_directo) {
                        ...
                        $write = fputs($fp, "D" . $FS . /* $formas_pago_descripcion[key($formas_pago)] */ strtoupper($este) . $FS . $formas_pago[1] . $FS . "T" . $FS . $formas_pago[0] . $LF);
                    }
                    $write = fputs($fp, "E" . $LF);
                    $write = fputs($fp, "{" . $LF);
                    fclose($fp);
                ...
                case "bixolon":
                    ...
                    $tasa_impuesto = array(/* Exento */$index_0 => "d0", /* 8 */ $index_1 => "d2", /* 12 */ $index_2 => "d1", /* 22 */ $index_3 => "d3");
                    ...
                    $sql = "UPDATE factura_devolucion SET cod_devolucion_fiscal = ".$nro_factura_fiscal." WHERE id_devolucion = ".$id_facturaTrans."";
                    $factura_devolucion->ExecuteTrans($sql);
                    ...
                    $cabecera = "<ABRIR_DEVOLUCION," .str_replace(",","",$dataDevolucion[0]['nombre']). "," .$dataDevolucion[0]['rif']. "," .$_POST['numeroF']. "," .$_POST['serialF']. "," .$dataDevolucion[0]['fechaFactura']. "," .$dataDevolucion[0]['hora'].">\n";
                    ...
                    foreach ($lineas as &$linea) {
                        ...
                        $detalles = "<ITEM_CF," .strtoupper($linea["descripcion"]). "," .($linea["cantidad"]*1000). "," .number_format($linea["precio"],2,"",""). "," .$iva . "," . "0" . "," . "0" . ">" . "\n";
    ...
    $factura_devolucion->CommitTrans(1);
    $factura->CommitTrans(1);
