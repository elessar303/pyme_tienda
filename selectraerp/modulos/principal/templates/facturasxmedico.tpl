...
{else}
    ...
    {else}
        <tr  bgcolor="#e1e1e1">
    {/if}
        <td >{$campos.cod_edocuenta}</td>
        <td >{$campos.monto}</td>
	<td >{$campos.fecha_creacion}</td>
        <td >{$campos.descripcion}</td>
        <td>

	<img style="cursor: pointer;" class="imprimir" onclick="javascript: window.location.href='../../reportes/rpt_detalle_pago_medicos.php?id_proveedor={$campos.id_proveedor}&cod_edocuenta={$campos.cod_edocuenta}';" title="Imprimir Relacion de Convenio"  src="../../libs/imagenes/ico_print.gif">
