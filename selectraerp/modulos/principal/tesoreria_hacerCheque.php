...
require_once "../../libs/php/clases/cheque.php";
$comunes = new Comunes();
...
if (isset($_POST["opt"]) == true) {
    ...
    if ($opt_ == "guardarCheque") {
         ...
         * ESTE UPDATE FUE MODIFICADO para poder permitir que al poder cambiar el número de cheque
         * se guardara correctamente comparando con el número de cheque y no con el identificador
         */
        $update = "update cheque set monto = '" . $monto . "', ref = '" . $ref . "', id_proveedor = '" . $id_proveedor . "',";
        $update .= " concepto = '" . $concepto . "', situacion = 'Ac', fecha = '" . $fecha_pago . "'";
        #$update .= " where cod_cheque = " . $cod_cheque;
        $update .= " where cheque = " . $num_cheque;
        $cheque->ExecuteTrans($update);
        # SE CAMBIA DE ESTADO DE LA CHEQUERA AL CONSUMIR TODOS LOS CHEQUES
        $cheque1 = $_POST["codigo_cheque"];
        $chequee = ($_POST["codigo_cheque"] + 1);

        $cheque11 = $cheque->ObtenerFilasBySqlSelect("select * from cheque where cod_cheque =" . $cheque1);
        $cheque12 = $cheque->ObtenerFilasBySqlSelect("select * from cheque where cod_cheque =" . $chequee);

        $campo1 = $cheque11[0]["cod_chequera"];
        $campo2 = $cheque12[0]["cod_chequera"];

        if ($campo1 != $campo2) {
            $cheque->ExecuteTrans("update chequera set situacion = 'C' where cod_chequera =" . $campo1);
        }
...
//     WHERE timp.numero_control_factura = '".$ref."'
...
//                         VALUES (".$cod_cheque.", ".$registro["totalizar_monto_retencion"].", 'c', '".$fecha_pago."', '".$registro["descr_tipo_impuesto"]."', CURRENT_TIMESTAMP, '".$login->getUsuario()."','".$registro["cuenta_contable"]."');";
...
//     }
        ...
        SELECT banco.*, terbandet.cod_tesor_bandodet, terbandet.nro_cuenta, terbandet.cuenta_contable
        FROM cheque che
        ...
        $sql = "
        INSERT INTO `cheque_bache_det` (`cod_cheque`, `monto`, `tipo`, `fecha`, `descripcion`, `fecha_creacion`, `usuario_creacion`,cuenta_contable)
        VALUES (" . $cod_cheque . ", " . $monto . ", 'c', '" . $fecha_pago . "', '" . $detalle_banco[0]["descripcion"] . " Cta. " . $detalle_banco[0]["nro_cuenta"] . "', CURRENT_TIMESTAMP, '" . $login->getUsuario() . "','" . $detalle_banco[0]["cuenta_contable"] . "');";
        $cheque->ExecuteTrans($sql);
        $monto_debito +=$monto;
        /*
         * </Asiento de monto cheque contra banco.>
         */
        $prov = $cheque->ObtenerFilasBySqlSelect("
...
//    provee.id_proveedor = cxp.id_proveedor  inner join cxp_edocuenta_detalle cxpd on (cxpd.cod_edocuenta=cxp.cod_edocuenta)
// WHERE cxpd.numero = '".$ref."'");
...
//     $beneficiario = $cabEdoCuenta[0]["beneficiario"];
    ...
    INSERT INTO `cheque_bache_det` (
        ...
        `fecha_creacion`,
        `usuario_creacion`,cuenta_contable)
        VALUES (" . $cod_cheque . ", " . $monto . ", 'd', '" . $fecha_pago . "', '" . $proveedor . "', CURRENT_TIMESTAMP, '" . $login->getUsuario() . "','" . $cuenta_contableProv . "');";
        $cheque->ExecuteTrans($sql);
...
# SE CAMBIA DE ESTADO DE LA CHEQUERA AL CONSUMIR TODOS LOS CHEQUES
        ...
        if ($campo1 != $campo2) {
            $sql2 = "update chequera set situacion = 'C' where cod_chequera =" . $campo1;
            $cheque->ExecuteTrans($sql2);
        }
        /*
         * <verificamos si existen impuestos para cargarlos en bauche det_>
         */
...
# SE CAMBIA DE ESTADO DE LA CHEQUERA AL CONSUMIR TODOS LOS CHEQUES
    ...
    INSERT INTO `cheque_bache_det` (
        ...
        WHERE cxp.numero = '" . $Array_num_compra . "'");
             ...
             * <CAMBIAMOS EL ESTADO DE LA CUENTA POR COBRAR Y LA DEL ASIENTO DEL CHEQUE A LA LETRA <<X>> >
             */
            $sql3 = $cheque->ObtenerFilasBySqlSelect("SELECT * FROM cheque WHERE cod_cheque =" . $_POST["codigo_cheque"]);
            ...
            for ($ii = 0; $ii < $cantFact; $ii++) {
                ...
                $update = "update cxp_edocuenta inner join cxp_edocuenta_detalle on
	      cxp_edocuenta_detalle.cod_edocuenta = cxp_edocuenta.cod_edocuenta
	     and cxp_edocuenta_detalle.marca = 'P'
	     set
	     cxp_edocuenta.marca = 'X',
	     cxp_edocuenta_detalle.marca = 'X'
	    where cxp_edocuenta_detalle.numero = '" . $facturass[$ii] . "'";
                $cheque->ExecuteTrans($update);
            }
            /*
             * </CAMBIAMOS EL ESTADO DE LA CUENTA POR COBRAR Y LA DEL ASIENTO DEL CHEQUE A LA LETRA <<X>> >
