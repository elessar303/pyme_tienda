<?php /* Smarty version 2.6.21, created on 2018-10-02 15:31:58
         compiled from cataporte_deposito_add.tpl */ ?>
<?php require_once(SMARTY_CORE_DIR . 'core.load_plugins.php');
smarty_core_load_plugins(array('plugins' => array(array('modifier', 'date_format', 'cataporte_deposito_add.tpl', 201, false),array('modifier', 'number_format', 'cataporte_deposito_add.tpl', 203, false),)), $this); ?>
<!DOCTYPE html>
<html>
    <head>
 ...
 ?>
        ...
        <script language="JavaScript" type="text/JavaScript">
            ...
            function generardeposito() 
            {
                ...
                parametros = {"arreglo" : checkboxValues, "opt" : "calcular_monto_cataporte_deposito"}
                $.ajax(
                {
                    type: "POST",
...
<?php echo ';
                ...
                {
                    alert(\'Debe Ingresar el Nro de Referencia\');
                    document.form1.nro_cataporte.focus();
                    exit;
                }
                if(document.form1.banco.value==\'\' )
                {
                    alert(\'Debe Ingresar Banco - Cuenta\');
                    document.form1.cantidad.focus();
                    exit;
                }
                if(document.form1.cat_val.value==1 )
                {
                    alert(\'Debe Ingresar un Nro de Referencia No Usado\');
                    document.form1.nro_cataporte.focus();
                    exit;
                }
                if(document.form1.id_principal.value==\'\')
                {
                    alert(\'Error, Falta Dato Principal, consulte al Adminsitrador\');
                    document.form1.nro_cataporte.focus();
                    exit;
                }
                /*for(var i = 1; i<=nro; i++)
...
unset($_smarty_tpl_vars);
 ?>
                <table style="width:50%; background-color:white;" cellpadding="1" cellspacing="1" class="seleccionLista" align="center">
                    <thead>
                        <tr>
                            <th class="tb-head" style="text-align:center;" colspan="5">Generar Cataporte deposito para Punto de Venta: <?php echo $this->_tpl_vars['parametros'][0]['codigo_siga']; ?>
</th>
                        </tr>       
                        <tr>
                        	<td align="left">N°</td>
                        	...
                        	<td align="center">Monto Deposito </td>
...
?>
                		<tr>
        	                ...
        	                <td align="center"><?php echo ((is_array($_tmp=$this->_tpl_vars['dato']['fecha_creacion'])) ? $this->_run_mod_handler('date_format', true, $_tmp, "%d-%m-%Y") : smarty_modifier_date_format($_tmp, "%d-%m-%Y")); ?>
</td>
        	                <td align="center"><?php echo $this->_tpl_vars['dato']['nro_deposito']; ?>
</td>
                            <td align="center"><?php echo ((is_array($_tmp=$this->_tpl_vars['dato']['monto'])) ? $this->_run_mod_handler('number_format', true, $_tmp, 2, ".", "") : number_format($_tmp, 2, ".", "")); ?>
</td>
        	                <td align="center">
                                <?php if ($this->_tpl_vars['dato']['monto'] <= '0.00'): ?>
                                    <p>Sin deposito <p>
                                <?php else: ?>
                                    <input type="radio" value="<?php echo $this->_tpl_vars['dato']['id']; ?>
" name="id_depositos[]" id="id_depositos[]" onclick="generardeposito();" />
                                <?php endif; ?>
        ...
        <br>
        <form method='POST' action='generar_cataporte_fin_deposito.php' name='form1' >
