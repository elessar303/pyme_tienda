...
if($num[0]['contar']==0)
{
    ...
    $insertC=$conn->ExecuteTrans("insert into comprobante (caja, banco, id_usuario) values ('".$monto_usuario."', '".$monto_usuario."', '".$login->getIdUsuario()."')");
    ...
    $insertar2=$conn->ExecuteTrans(
        "INSERT INTO 
        cataporte (nro_cataporte, cant_bolsas, tipo_cataporte, fecha, cod_usuario, monto_usuario, observacion) 
        VALUES 
        ('".$nro_cataporte."',".$count.", '5', '".$fecha."',".$_SESSION['cod_usuario'].", ".$monto_usuario.", ".$observacion.")
        ");
	foreach ($_POST['nro_depositos'] as $key) 
    {
        $num1=$conn->ObtenerFilasBySqlSelect("Select monto_acumulado_credito from caja_principal where nro_deposito='".$key."'");
        $acumuladototal=$num1[0]['monto_acumulado_credito']-$monto_usuario;
        if($monto_usuario==0)
        {
        ...
        $insertar3=$conn->ExecuteTrans(
            ...
            (select (id_deposito+1), '".$correlativo."', monto, monto_acumulado, '".$nro_cataporte."', now(), cod_banco, '".$login->getNombreApellidoUSuario()."', monto_tarjeta, monto_acumulado_tarjeta, monto_deposito, monto_acumulado_deposito,   monto_tickets, monto_acumulado_tickets, monto_cheque, monto_acumulado_cheque,  ".$monto_usuario.", ".$acumuladototal."  from caja_principal where nro_deposito='".$key."')
            ");
