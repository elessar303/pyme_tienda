...
$consulta = "SELECT * FROM nota_entrega ne
    INNER JOIN nota_entrega_detalle ned ON ned.id_nota_entrega = ne.id_nota_entrega
    INNER JOIN clientes c ON c.id_cliente = ne.id_cliente
    WHERE ne.cod_nota_entrega = '{$_GET["codigo"]}';";
...
$consulta = "SELECT * FROM nota_entrega_detalle WHERE id_nota_entrega = '{$dataDevolucion[0]["id_nota_entrega"]}';";
...
if (isset($_POST["anular"])) {
    if ($dataDevolucion[0]["cod_estatus"] == 1) {
        ...
        $factura->ExecuteTrans("UPDATE nota_entrega SET cod_estatus = '3' WHERE id_nota_entrega = " . $dataDevolucion[0]["id_nota_entrega"]);
        ...
        for ($i = 0; $i < $cantItems; $i++) {
            ...
            VALUES (
                ...
                '" . $dataDevolucion[$i]["_item_almacen"] . "', '" . $dataDevolucion[$i]["cod_item"] . "', '" . $dataDevolucion[$i]["_item_cantidad"] . "');";
...
#echo "transaccion: ".$id_transaccion." codigo:".$dataDetallePedido[$i]["id_item"]."alamacen: ".$dataDetallePedido[$i]["_item_almacen"]." cantidad: ".$dataDetallePedido[$i]["_item_cantidad"]."<br>";
            ...
            $campos = $factura_devolucion->ObtenerFilasBySqlSelect("
                        select * from item_existencia_almacen
                                where id_item  = '" . $dataDevolucion[$i]["cod_item"] . "' and
        ...
        where id_item  = '" . $dataDevolucion[$i]["cod_item"] . "' and cod_almacen = '" . $dataDevolucion[$i]["_item_almacen"] . "'");
