...
// si el usuario hizo post
    ...
    {# Verificar que no se trata de una compra con estatus de entrega de productos "Pendiente"
        ...
        $kardex_almacen_instruccion = "INSERT INTO kardex_almacen (
            ...
            `fecha_creacion`, `estado`, `fecha_ejecucion`, `empresa_transporte`, `id_conductor`, `placa`, `guia_sunagro`, `orden_despacho`)
        VALUES (
            NULL , '3', '{$_POST["autorizado_por"]}',
            '{$_POST["observaciones"]}', '{$_POST["input_fechacompra"]}', '{$login->getUsuario()}', 
            CURRENT_TIMESTAMP, 'Entregado', CURRENT_TIMESTAMP, '{$_POST["empresa_transporte"]}', '{$id_conductor[0]["id_conductor"]}', '{$_POST["placa"]}', '{$_POST["codigo_sica"]}', '{$_POST["orden_despacho"]}');";
