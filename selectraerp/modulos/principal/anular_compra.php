...
$consulta = "SELECT * FROM compra ne
    INNER JOIN compra_detalle ned ON ned.id_compra = ne.id_compra
    INNER JOIN proveedores prov  c ON c.id_proveedor= ne.id_proveedor
    WHERE ne.cod_compra = '{$_GET["codigo"]}';";
...
//$consulta = "SELECT * FROM compra WHERE id_compra = '{$dataDevolucion[0]["id_compra"]}';";
...
$smarty->assign("name_form","anular_compra");
...
if (isset($_POST["anular"])) {
    if ($dataDevolucion[0]["cod_estatus"] == 1) {
        ...
        $factura->ExecuteTrans("UPDATE compra SET cod_estatus = '3' WHERE id_compra = " . $dataDevolucion[0]["id_compra"]);
