...
$sql="SELECT 
    ifnull(date_format(max(fecha_final),\"%m-%Y\"),'--/---- (ningun registro de conciliación)')  as periodo
        FROM `conciliacion_bancaria` where `cod_tesor_bancodet` = ".$_GET["cod_cuenta"];

$periodo = $comunes->ObtenerFilasBySqlSelect($sql);
$smarty->assign("periodo",$periodo[0]["periodo"]);
