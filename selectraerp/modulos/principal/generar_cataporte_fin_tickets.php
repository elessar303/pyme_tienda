...
if($num[0]['contar']==0)
{
    ...
    $insertC=$conn->ExecuteTrans("insert into comprobante (caja, banco, tipo_mov) values ('".$monto_usuario."', '".$monto_usuario."', 'TRDPM' )");
    ...
    $insertar2=$conn->ExecuteTrans(
        ...
        VALUES 
        ('".$nro_cataporte."',".$count.", '1', '".$fecha."',".$_SESSION['cod_usuario'].", ".$monto_usuario.", ".$observacion.", '".$_POST['banco']."')
        ");
	foreach ($_POST['nro_depositos'] as $key) 
    {
        $num1=$conn->ObtenerFilasBySqlSelect("Select monto_acumulado_tickets from caja_principal where nro_deposito='".$key."'");
        $acumuladototal=$num1[0]['monto_acumulado_tickets']-$monto_usuario;
        if($monto_usuario==0)
        {
        ...
        $insertar3=$conn->ExecuteTrans(
            ...
            (select (id_deposito+1), '".$correlativo."', 0, monto_acumulado, '".$nro_cataporte."', now(), cod_banco, '".$login->getNombreApellidoUSuario()."', 0, monto_acumulado_tarjeta, 0, monto_acumulado_deposito, ".$monto_usuario.", ".$acumuladototal.", 0, monto_acumulado_cheque, 0, monto_acumulado_credito  from caja_principal where nro_deposito='".$key."')
            ");
