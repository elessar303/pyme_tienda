...
$tabla = "vista_chequera_lista";
$name_form = "vw_chequera_lista";
$tipob = @$_GET['tipo'];
$des = @$_GET['des'];
$pagina = @$_GET['pagina'];
...
if (isset($_GET["cod_cuenta"])) {
    $campos = $comunes->ObtenerFilasBySqlSelect("
                select tb.cod_tesor_bandodet, tb.cod_banco, tb.nro_cuenta,
...
if (isset($_POST['buscar']) || $tipob != NULL) {
    ...
    $instruccion .= " and cod_tesor_bandodet = " . $_GET["cod_cuenta"] . " and situacion = 'A'";
} else {
    $instruccion = "SELECT * FROM  " . $tabla . " where cod_tesor_bandodet = " . @$_GET["cod_cuenta"] . " and situacion = 'A'";
}
...
$smarty->assign("cabecera", array("C&oacute;digo", "Situaci&oacute;n", "Cantidad", "Inicio", "Fin"));
...
//**************************************************************************
$smarty->assign("option_values", array("cod_chequera", "situacion", "nro_cuenta"));
$smarty->assign("option_output", array("C&oacute;d. Chequera", "Situaci&oacute;n", "Nro. Cuenta"));
...
$sqlcheques_generados = "
        select if(che.cod_chequera,'si','no') as cheques_generados, vw.cod_chequera
        from vista_chequera_lista vw left join cheque che on che.cod_chequera = vw.cod_chequera
        where  vw.cod_chequera group by vw.cod_chequera";
$cheques_generados = $comunes->ObtenerFilasBySqlSelect($sqlcheques_generados);

$validarChequesGenerados = array();
foreach ($cheques_generados as $valor) {
