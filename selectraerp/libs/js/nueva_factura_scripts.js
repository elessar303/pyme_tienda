...
$(document).ready(function(){
    ...
    **/
    function addTabla(codigo, descripcion, cantidad, preciosiniva, descuento, montodescuento, totalsiniva, iva, piva, totalconiva, almacen, producto,ubicacion, cuota/*Adicionado como requisito especifico del CCP*/){
        ContarInputItem += 1;
        campos = "";
        ...
        if ($("input[name='cod_item_forma']").val() == 1) { // Si es un Producto
            $.ajax({
                type: "GET",
                url:  "../../libs/php/ajax/ajax.php",
                data: "opt=precomprometeritem&v1="+codigo+"&cpedido="+cantidad+"&codalmacen="+almacen+"&ubicacion="+ubicacion+"&codprecompromiso="+ContarInputItem,//+"&tipo_transaccion="+transaccion,
                beforeSend: function(){
    ...
    **/
    $("#addTabla").click(function(){
        ...
        addTabla(codigo,descripcion,cantidad,preciosiniva,descuento,montodescuento,totalsiniva,iva,piva,totalconiva,almacen, producto,ubicacion);
    });
...
function VerificarCantidad(){    
        ...
        codAlmacen = $("#cod_almacen").val();
        ubicacion = $("#ubicacion").val(); 
        codItem = $("input[name='id_item']").val();  
        if( codAlmacen !='' && ubicacion !='' && $("input[name='cod_item_forma']").val() == 1 ){
          
            $.ajax({
                type: "GET",
                url:  "../../libs/php/ajax/ajax.php",
                data: "opt=verificarExistenciaItemByAlmacen&v1="+codAlmacen+"&v2="+codItem+"&ubicacion="+ubicacion,
                beforeSend: function(){
                   ...
                   resultado = eval(data)
                    if(resultado[0].id=="-1"){
                        ...
                        return false;
                         ...
                         $("#cantidadExistente").val(resultado[0].cantidad);
    ...
    $.limpiarCamposFiltro  = function () {
        ...
        $("input[name='filtro_cantidad']").val("0.00");
                ...
                callback: function(records, operation, success) {
                    if (success) {
                        if(records.length > 1){
                            ...
                            productoSeleccionado = records[0].json;
                            $("input[name='filtro_descripcion']").val(productoSeleccionado.descripcion1);
                            $("input[name='filtro_referencia']").val(productoSeleccionado.codigo_barras);
                            $("input[name='filtro_precio']").val(productoSeleccionado.precio1);
                            $("input[name='filtro_importe']").val(productoSeleccionado.utilidad1);
    ...
    $("input[name='filtro_cantidad']").keypress(function(ev){
        ...
        if(teclaTabMasP == codeCurrent){
            $.cargarItem({
                ...
                tipo_item:          productoSeleccionado.cod_item_forma,
                cod_item:           productoSeleccionado.cod_item,
                descripcion_item:   productoSeleccionado.descripcion1
            });
            $.limpiarCamposFiltro();
        }
    });
    ...
    $.cargarItem = function(parametros){
        ...
        $("#descripcion_item").attr("style", "font-weight: bold; font-size: 15px;");
        //$("#descripcion_tipo_forma").attr("style", "font-family:'Verdana'; font-weight: bold;");
        $("#descripcion_tipo_forma").attr("style", "font-family:'Verdana';");
        valor = item;
        if(valor==''){
    ...
    function cargarCantidad(){    
        ...
        ubicacion = $("#ubicacion").val(); 
        item=valor;       
        if( codAlmacen !='' && ubicacion !='' ){
           
            $.ajax({
                type: "GET",
                url:  "../../libs/php/ajax/ajax.php",
                data: "opt=CargarAlmacenesDisponiblesByIdItem&v1="+valor+"codAlmacen="+codAlmacen+"ubicacion="+ubicacion,
                beforeSend: function(){
    ...
    // $("#cod_almacen").change(function(event) {
    //     cargarCantidad();
    // });
    ...
    // });
            ...
            //         }else{
            //             $("#cod_almacen").find("option").remove();
            //             $("#cod_almacen").append("<option value=''>Seleccione...</option>");
        ...
        $.ajax({
            type: 'POST',
            data: 'opt=CargarAlmacenes',
            url: '../../libs/php/ajax/ajax.php',
            beforeSend: function() {
                $("#cod_almacen").find("option").remove();
                $("#cod_almacen").append("<option value=''>Cargando..</option>");
            },
            success: function(data) {
                $("#cod_almacen").find("option").remove();
                ...
                for (i = 0; i <= this.vcampos.length; i++) {
                    $("#cod_almacen").append("<option value='"+this.vcampos[i].cod_almacen+"'>" + this.vcampos[i].descripcion + "</option>");
                }
            }
        });
    }
